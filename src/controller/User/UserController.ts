...
import { UserPostType } from "../../types/userType.js";
import { ListPermissionsType } from "../../types/permissionType.js";
import { ListRolessType } from "../../types/rolesType.js";

const SERVICE = 1;

export default class UserController {
  static cadastrarUser = async (request: FastifyRequest<{ Body: { user: UserPostType } }>, reply: FastifyReply) => {
    try {
      const userTarget = request.body.user;

      const response = await permission_api.get("/roles");
      const permissions: ListPermissionsType = response.data;
  ...
  static getOneUser = async (request: FastifyRequest, reply: FastifyReply) => {
    try {
  ...
  static getAllUser = async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const user = request.user;
  ...
  static atualizarUser = async (request: FastifyRequest<{ Body: { user: UserPostType }, Params: { id: string } }>, reply: FastifyReply) => {
    try {
      const id = request.params.id;
      const userTarget = request.body.user;
      const user = request.user;
      ...
      const response = await permission_api.get("/roles");
      const roles: ListRolessType = response.data;

      const isValid = roles.some((role) => {
        return role.id == userTarget.role_id;
      });
  ...
  static deletarUser = async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const id = request.params.id;
      const user = request.user;
