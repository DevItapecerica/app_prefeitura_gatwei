export const DataFormat = (data) => {
  ...
  const bolsistasHeader = [
    'bco', 'ag', 'dig_ag', 'conta', 'dig_conta', 'nome', 'valor', 'vencimento', 'cpf', 'local'
  ];
  ...
  for (const local in agrupados) {
    ...
    bolsistasRows.push(['Total de bolsistas local:', rows.length, 'Total do local:', totalValor]);
    totalGeral += totalValor;
    contagemGeral += rows.length
  }

    bolsistasRows.push(['Total de bolsistas geral:', contagemGeral, 'Total do valor geral:', totalGeral]);
